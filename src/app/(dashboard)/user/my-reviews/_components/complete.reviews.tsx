import { useCompletedReviews } from "../_hooks/use.completed.reviews";
import LoadingData from "@/components/ui/loading.data";
import { Button } from "@/components/ui/button";
import { Ban, CircleX } from "lucide-react";
import CompletedReviewCard from "./complete-review-components/completed.review.card";

export default function CompleteReviews() {
  const { completedReviews, isLoading, isError, error, refetch } =
    useCompletedReviews();
    ...
    <section>
      {completedReviews && completedReviews.length > 0 ? (
        completedReviews.map((review) => (
          <CompletedReviewCard key={review.booking_uid} review={review} />
        ))
      ) : (
        <div className="flex flex-col items-center justify-center gap-4 py-12 text-gray-500">
          ...
          <p className="text-lg">No completed reviews found</p>
        </div>
      )}
    </section>
  );
